...
const Card = () => {
      ...
      {userData ? (
        <>
          ...
          <CompactCard type="merd" param={userData} onClick={() => toggleExpanded("merd")} />
             {/* Table displaying density, fertility rate, and median age */}
  ...
  <table className="table">
    ...
    <tbody>
      {userData.countriesWithData.map((country, index) => (
        <tr key={index}>
          <td>{country["Country (or dependency)"]}</td>
          <td>{country.density}</td>
          <td>{country.fertilityRate}</td>
          <td>{country.merdRate}</td>
        </tr>
      ))}
    </tbody>
  </table>
